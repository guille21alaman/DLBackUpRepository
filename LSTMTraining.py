...
describeTargets(test, file=None, normalize=True) # Function located in HelperFunctions.py
# print("\n#############################################\nShow X_train:\n#############################################\n")
...
# Until here identical to ML_Training
    ...
    for target in range(Ytrain[stock].shape[1]):
        ...
        for c in results.columns:
            if column_name in c:
                ...
                elif ("1.5" in  Ytrain[stock].columns[target]) & (("Precision" in c) or ("Preicision" in c)):
                    list_evaluation_columns.append(c)
                else:
                    continue
        ...
        model.add(Dense(units=units_dense*2, activation = "relu"))
        model.add(Dense(units=units_dense, activation = "relu"))
        model.add(Dropout(dropout))
        model.add(Dense(units = units_last_dense, activation="softmax"))
        ...
        print("#########################################\nTraining starts...\n#############################################") 
        target_train = keras.utils.np_utils.to_categorical(original_y_train.iloc[n:,target] -1) #-1 to be consistent with keras class organization 0,1,2...,n
        target_test = keras.utils.np_utils.to_categorical(original_y_test.iloc[n:,target]- 1)

        # print("Eventually, the target looks as follows: ")
        # print("y_train: ", target_train, "\nwith shape: ", target_train.shape)
        
        # #TODELETE #TODO
        # model = keras.models.load_model('results/AAPL/DL_Results/model/round_3_LSTM_5Days_[-inf, 0, inf]_accuracy_0.5')
        ...
        # print(confusion_matrix(predicted_stock_price_labels, target_test_labels))
        # exit()
        ...
        model.fit(X_train, target_train, epochs = epochs, batch_size = batch_size,
            ...
            class_weight=class_weights) #note that under epochs you can see how many batches were processed so far
        print("\n\n")
